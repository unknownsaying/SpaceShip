...
Public Class MobiusSphereRenderer
    ...
    Public Sub New()
        Me.mobius = New SphericalMobiusRing(1.5, 2.5, 3.14)
    ...
    Private Sub DrawMobiusInSphericalCoords(g As Graphics)
        ...
        For i As Integer = 0 To uSteps - 1
            ...
            For j As Integer = 0 To vSteps - 1
                ...
                Dim x3d As Double = spherical.r * Sin(spherical.theta) * Cos(spherical.phi)
                Dim y3d As Double = spherical.r * Sin(spherical.theta) * Sin(spherical.phi)
                Dim z3d As Double = spherical.r * Cos(spherical.theta)
    ...
    Private Function RotatePoint(point As Point3D, angleX As Double, angleY As Double) As Point3D
        ...
        Dim x1 As Double = point.X * Cos(angleY) - point.Z * Sin(angleY)
        Dim z1 As Double = point.X * Sin(angleY) + point.Z * Cos(angleY)
        
     
        Dim y2 As Double = point.Y * Cos(angleX) - z1 * Sin(angleX)
        Dim z2 As Double = point.Y * Sin(angleX) + z1 * Cos(angleX)
