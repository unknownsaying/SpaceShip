...
Public Class KleinBottle
    ...
    Private Sub DrawKleinBottle(g As Graphics)
        ...
        For i As Integer = 0 To uSteps - 1
            ...
            For j As Integer = 0 To vSteps - 1
                Dim v As Double = (j / vSteps) * 2 * Math.PI
                ...
                ' color
                Dim red As Integer = CInt((u / (2 * Math.PI)) * 255)
                Dim green As Integer = CInt((v / (2 * Math.PI)) * 255)
                Dim blue As Integer = CInt(128 + 127 * Math.Sin(u + v))
    ...
    Private Function Rotate3D(point As Point3D, rx As Double, ry As Double, rz As Double) As Point3D
        ' X-axis
        Dim y1 As Double = point.Y * Math.Cos(rx) - point.Z * Math.Sin(rx)
        Dim z1 As Double = point.Y * Math.Sin(rx) + point.Z * Math.Cos(rx)
        ' Y-axis
        Dim x2 As Double = point.X * Math.Cos(ry) + z1 * Math.Sin(ry)
        Dim z2 As Double = -point.X * Math.Sin(ry) + z1 * Math.Cos(ry)
        ' Z-axis
        Dim x3 As Double = x2 * Math.Cos(rz) - y1 * Math.Sin(rz)
        Dim y3 As Double = x2 * Math.Sin(rz) + y1 * Math.Cos(rz)
